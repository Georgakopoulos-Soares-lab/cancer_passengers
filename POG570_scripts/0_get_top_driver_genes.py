...
# input files/dir
DRIVER_GENES_INTOGEN = "data/POG570/driver_genes_intogen"
REF_GENOME = "data/ref/hg19.fa"
GENCODE_ANNOT = "data/ref/gencode.v19.annotation.gtf" # for getting start and end positions of genes
CHROM_SIZES = "data/ref/hg19.chrom.sizes"
REFGENE_ANNOT = "data/ref/hg19.refGene.gtf" # for getting genic region annotations
INTRON_ANNOTATION = "data/ref/hg19_gene_annot_introns.bed"

# output files/dir
GENE_ANNOT_BED_DIR = "data/POG570/genic_region_annot_bed"
DRIVER_GENES_DIR = "data/POG570/driver_genes"
GENE_LIST = "metadata/POG570_gene_list.tsv"
...
CANCER_TYPES = os.listdir(DRIVER_GENES_INTOGEN)
CANCER_TYPES = [cancer_type.replace(".tsv", "") for cancer_type in CANCER_TYPES]
# remove hidden files
CANCER_TYPES = [cancer_type for cancer_type in CANCER_TYPES if not cancer_type.startswith(".")]
...
for cancer_type in CANCER_TYPES:
	driver_genes_intogen = pd.read_csv(f"{DRIVER_GENES_INTOGEN}/{cancer_type}.tsv", sep="\t")
	...
	driver_genes_top = driver_genes_intogen[driver_genes_intogen["Samples (%)"] >= 2]
	driver_genes_top.drop_duplicates(subset=["Symbol"], inplace=True, keep="first")
	# get top 10 driver genes, include genes with the same percentage as the 10th gene
	top_percentages = driver_genes_top["Samples (%)"].head(10)
	driver_genes_top = driver_genes_top[driver_genes_top["Samples (%)"].isin(top_percentages)]
	...
	tert_promoter_cancers = ['BLCA', 'CNS-PNS', 'HNSC', 'HCC', 'SKCM', 'THCA']
